...
- **Admin rozhranie – hotové (nad rámec pôvodného plánu):** `/admin` v Next.js appke, tmavý štýl podľa knižnice komponentov. Účty s heslami (scrypt), session tokeny v httpOnly cookie, roly owner/staff, rate limit na prihlásenie. Sekcie: kalendár obsadenosti, rezervácie (zoznam, detail, ručné storno), katalóg (izby, služby, sezónne ceny, zdroje a pracovný čas), používatelia. Detaily v EDIT-LOG.md.
